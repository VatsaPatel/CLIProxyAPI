...
func (e *ClaudeExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if claudeOpenRouterRedactedThinkingRepairEnabled(baseURL, baseModel) {
		if stream {
			data = repairClaudeOpenRouterSSEStream(data)
		} else {
			data = repairClaudeOpenRouterJSONResponse(data)
		}
	}
	...
	go func() {
		...
		if responseFormat == to {
			...
			flushEvent := func() bool {
				...
				if claudeOpenRouterRedactedThinkingRepairEnabled(baseURL, baseModel) {
					var ok bool
					cloned, ok = repairClaudeOpenRouterSSEFrame(bytes.TrimSuffix(cloned, []byte("\n")), dropped, indexMap, &nextIndex)
					if !ok {
						return true
					}
